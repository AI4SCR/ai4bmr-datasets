...
class IMCDataset(BaseDataset):
    ...
    def __init__(
        ...
        self.measurements_dir = (
            self.base_dir / "measurements" / self.measurement_version
        )
        ...
        self.clinical_metadata_path = (
            self.base_dir / "metadata" / "02_processed" / "clinical_metadata.parquet"
        )
        self.mcd_metadata_path = (
            self.base_dir / "metadata" / "02_processed" / "mcd_metadata.parquet"
        )
        self.labels_path = (
            self.base_dir / "metadata" / "02_processed" / "labels.parquet"
        )
    ...
    def load(self):
        ...
        def process(df: pd.DataFrame):
            ...
            sample_names = df.index.get_level_values("mask_path").map(
            ...
            df.index = df.index.droplevel(
                list(set(df.index.names) - {"object_id", "sample_name"})
            )
            return df
        ...
        intensity, clinical_metadata = intensity.align(
            clinical_metadata, join="inner", axis=0
        )
        ...
        if self.labels_path.exists():
            ...
            index_levels_to_drop = list(
                set(labels.index.names) - {"object_id", "sample_name"}
            )
        ...
        for mask_path in self.masks_dir.glob("*.tiff"):
            ...
            masks[mask_path.stem] = Mask(
                id=mask_path.stem, data_path=mask_path, metadata_path=metadata_path
            )

        images = {}
        for image_path in self.images_dir.glob("*.tiff"):
            images[image_path.stem] = Image(
                id=image_path.stem, data_path=image_path, metadata_path=self.panel_path
            )
