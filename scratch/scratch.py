...
x = ds.load(
    image_version=version_name, mask_version=version_name, features_as_published=False
)
for sample in ds:
    sample_id = sample["sample_id"]
    intensity_path = (
        ds.get_intensity_dir(image_version=version_name, mask_version=version_name)
        / f"{sample_id}.parquet"
    )
    intensity = intensity_features(
        sample["image"].data, sample["mask"].data, sample["panel"]
    )
