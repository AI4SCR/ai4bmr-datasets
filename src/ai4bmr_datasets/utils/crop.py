...
def get_crops_from_image(
    ...
    for region in props:
        ...
        min_row, min_col = max(0, min_row - self.padding), max(
            0, min_col - self.padding
        )
        max_row, max_col = min(H, max_row + self.padding), min(
            W, max_col + self.padding
        )
